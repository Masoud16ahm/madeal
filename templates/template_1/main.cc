/*%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-
%             ma deal              %
%  Template for Small deformation  % 
%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-*/

//! Include the "small_def" header for elastic small deformation
#include <small_def.h>
...
int main(){
  ...
  //! (1) Enter Dimension and polynomial degree
  const int dim   = 2;
  const int p_deg = 1;

  //! (2) Define FEM object and Entering input file
  Elastic::Small_def<dim> problem1("inputfile.prm", p_deg);

  //! (3) Enforce any input manually
  problem1.output_name = "out"; //The output name
  problem1.s_typ       = 1;     //Solution type (1 plane-stress; 2 plane-strain)
